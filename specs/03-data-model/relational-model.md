...
> Reflejan la **migración aditiva** `packages/db/prisma/migrations/0002_phase5_recommendations/migration.sql`. Ver `docs/audit/phase-5-{spec-readiness,openapi-implementation-audit}.md`.

- **(d) `recommendations.type text NULL`** — columna añadida para el tipo de recomendación del API (`reduce_usage`/`tariff_review`/`inspect_device`…), no presente en el modelo Fase 1. Se valida por Zod en la app (no por enum nativo); permite mapear cada recomendación al subtype de la alerta de origen sin tocar enums existentes.
- **(e) `recommendations.estimated_saving_kwh numeric(14,4) NULL`** — columna añadida para el ahorro estimado en energía (heurístico: 10% del exceso observado de la alerta; `null` si no hay base reducible). Acompañada del CHECK `recommendations_saving_kwh_nonneg` (`estimated_saving_kwh IS NULL OR estimated_saving_kwh >= 0`), coherente con la no-negatividad del canon. El ahorro en CLP no se persiste como columna nueva: se deriva en backend vía `BillingService` (null sin tarifa, BR-031).
- **(f) Migración 0002 aditiva y no destructiva.** Solo añade las 2 columnas NULL + 1 CHECK a `recommendations`; **no altera enums ni datos existentes**. Aplicada con `migrate deploy` OK contra Neon dev.
- **(g) Tipo fino de alerta en `context.subtype`.** El canon mantiene `AlertType` con 4 valores (`anomaly`/`high_device`/`over_budget`/`offline`); el detalle fino de las 5 reglas (`high_consumption`/`projection_risk` → `over_budget`) se modela en el jsonb `alerts.context` (campo `subtype`), expuesto por el API como `metadata.subtype`. No se añadieron valores al enum `AlertType`.

## Notas de implementación Fase 6

> Reflejan la **migración aditiva** `packages/db/prisma/migrations/0003_phase6_control/migration.sql`. Ver `docs/audit/phase-6-{control-data-model-audit,openapi-implementation-audit}.md`.

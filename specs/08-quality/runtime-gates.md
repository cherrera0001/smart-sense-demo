...
- **Estado actual:** **PASS (web, Vercel aislado)** / **BLOCKED (API + Docker entorno)**.
  ...
  - **API staging (Fase 8.2):** **BLOCKED por credenciales** — Railway `whoami` Unauthorized, `RAILWAY_TOKEN` ausente (bash+win); `flyctl`/`render` ausentes. **No se desplegó (no se inventó).** Runbooks ejecutables listos (Railway/Render/VPS). Desbloqueo: `railway login` o `RAILWAY_TOKEN`.
  - **Docker build:** sigue **BLOCKED (entorno)** — Dockerfiles listos para CI; no ejecutado local. No afecta el PASS del código (CI).
...
- **Estado actual (staging):** **READY-BLOCKED (Fase 8.1 y 8.2)** — sin API de staging desplegada. En Fase 8.2 el bloqueo es **por credenciales Railway** (`whoami` Unauthorized, `RAILWAY_TOKEN` ausente bash+win; no se desplegó, no se inventó). Correr `API_BASE_URL=<STAGING_API_URL> pnpm smoke:api` cuando exista el host (runbooks Railway/Render/VPS en `docs/audit/phase-8-2-api-staging-railway.md`, `docs/deployment/phase-8-2-api-staging-manual-{render,vps}.md`).
- **Registrar en:** `docs/audit/phase-7-runtime-verification.md`, `docs/audit/phase-8-1-staging-smoke.md`, `docs/audit/phase-8-2-railway-auth.md`.
...
| GATE-DEPLOY-001 | Vercel web v2 / `docker build` / API staging | **PASS (web aislado)** / **BLOCKED (API+Docker)** | **Fase 8.2:** web `smartsense-web-v2` deploy READY (401 = Deployment Protection, no fallo de build); API staging BLOCKED por credenciales Railway; Docker BLOCKED por entorno |
| GATE-E2E-001 | `pnpm smoke:api` | **PASS (local)** / **READY-BLOCKED (staging)** | 7/7 local contra Neon dev; staging pendiente de API desplegada (Fase 8.2: Railway sin auth) |

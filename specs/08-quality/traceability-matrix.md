...
| GATE-E2E-001 (smoke-api) | `scripts/smoke-api.mjs` / `verify:staging` | sí | sí (7/7 local · 7/7 remoto) | **PASS (local)** · **PASS (remoto, Fase 8.4: API en Vercel Serverless `smartsense-api-v2.vercel.app`)** |
| GATE-DEPLOY-001/002 (Vercel web v2 + API Vercel Serverless) | `smartsense-web-v2` (Vercel aislado) · `smartsense-api-v2` (Vercel Serverless) | sí | sí (web READY · API `/health`+`/readyz` 200, smoke 7/7) | **PASS (web aislado, Fase 8.2)** · **PASS (API Vercel Serverless, Fase 8.4)**. Railway/Render/Fly/VPS/Docker para la API = **OBSOLETOS** |
...
>
> **Nota Fase 8.3 (Paquete de staging ejecutable, 2026-06-06, PARTIAL robusto):** se añaden los gates de despliegue **GATE-DEPLOY-002** (API staging externally verified — **BLOCKED por credenciales**: railway/fly/render/docker no disponibles/no auth; paquete `render.yaml` + `scripts/deploy-railway-staging.{sh,ps1}` + `verify:staging` listo, runtime **`tsx`**; no se desplegó, no se inventó), **GATE-DEPLOY-003** (Web v2 public/bypass — **build PASS** con logs `Next.js 15.5.19`/`Compiled successfully`/`Build Completed`; **acceso BLOCKED-by-protection** 401, pasos UI en Settings → Deployment Protection del proyecto aislado), **GATE-DEPLOY-004** (PR aprobado antes del merge — **BLOCKED**, PR #1 abierto sin merge) y **GATE-DEPLOY-005** (aprobación manual del cutover — **PENDIENTE/BLOCKED**). Aclaración: el build fallido `No Next.js` es de `smart-sense-demo` (branch-previews desde la raíz), inofensivo, no se toca. **Producción NO tocada.** Detalle: `08-quality/runtime-gates.md` (GATE-DEPLOY-002..005), `docs/audit/phase-8-3-{precheck,vercel-web-v2-access,api-staging-status,pr-update}.md`, `docs/deployment/phase-8-3-{render-blueprint,railway-staging,vps-api-staging}.md`.
>
> **Nota Fase 8.4 (API en Vercel Serverless — "todo en Vercel"):** la API (`apps/api`, Fastify) se desplegó como **Vercel Serverless Function**, proyecto Vercel **aislado** `smartsense-api-v2`, alias estable **`https://smartsense-api-v2.vercel.app`** (SSO/Deployment Protection desactivado; seguridad por JWT+RBAC propios). **Verificado:** `GET /health` 200, `GET /readyz` 200 (`db: ok`, Neon), **smoke E2E remoto 7/7 PASS** (`STAGING_API_URL=<url> pnpm verify:staging`). **Esto SUSTITUYE (OBSOLETO/superseded) a Railway/Render/Fly/VPS/Docker como host de la API** (GATE-DEPLOY-001/002 y GATE-E2E-001 remoto = **PASS**). CI verde en el último push (success), commit `d87765b` en `release/smartsense-f0-f7`; **PR #1 sin merge**; producción antigua `smart-sense-demo` NO tocada. Limitaciones serverless: rate-limit desactivado en la función (endurecer en edge/WAF), sin WebSocket, cold starts. Detalle: `docs/deployment/phase-8-4-vercel-serverless-api.md`.
>
> **Nota Fase 8.5 (Spec Compliance Gate):** reconciliación documental de los specs de gobierno con la realidad verificada de F0–F7 + Fase 8.4 (API en Vercel Serverless; tabla `refresh_tokens` #22 de Fase 7; enums nativos PostgreSQL). Sin cambios de código ni de `openapi.yaml`.

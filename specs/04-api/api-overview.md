...
## Estado de implementación (Fases 2–6)
...
> **Fase 5 — ✅ PASS (backend) (2026-06-04, `feat/phase-5-alerts-recommendations`).** Implementados y verificados (API **114/114** contra Neon real; 13 alerts + 8 recommendations + dashboard regresión) los **3 endpoints de alertas/recomendaciones**, todos bajo JWT:
> - **GET `/installations/{id}/alerts`** (`read`) — query `status`/`severity`/`from`/`to`/`limit` (default `status=open`); **lectura pura** (no evalúa). `items[{id,type,severity,status,title,message,device_id,detected_at,reviewed_at,metadata}]`, `installation_id`, `total`. `detected_at`=`created_at`, `metadata`=`context`.
> - **PATCH `/alerts/{id}/review`** — **RBAC `ROLES.operate`** (viewer → 403); `status` reviewed/dismissed + `note?`; setea `reviewed_at`/`reviewedBy`; `audit_log` action `'alert.review'`; **no borra** (append-only). → `{id,status,reviewed_at}`.
> - **GET `/installations/{id}/recommendations`** (`read`) — query `status` active/dismissed/applied/all (default `active`) + `limit`. `items[{id,alert_id,type,priority,title,message,estimated_saving_clp,estimated_saving_kwh,status,created_at}]`, `installation_id`, `total`.
> - **Motor interno `AlertEvaluationService.evaluateInstallationAlerts`** (no endpoint público; scheduler → fase posterior): **5 reglas basadas en evidencia** con dedup (ventana 24h) — `high_consumption`/`projection_risk`→`over_budget`(subtype), `device_high`→`high_device`, `offline`, `anomaly`; sin evidencia/baseline → no alerta; **0 side-effects** (no crea recommendations/control_actions). **`RecommendationService.generateForAlert`**: 1 recomendación/alerta (`source='alert'`); `estimated_saving_kwh`=10% del exceso observado (null si no reducible); `estimated_saving_clp` vía BillingService (null sin tarifa — BR-031); textos sin "garantizado". **`DashboardService.alerts_pending_count`** ahora **real** (count `alerts status=open`; antes literal 0).
>
>   **Migración aditiva 0002** (`recommendations.type` text + `estimated_saving_kwh` numeric(14,4) + CHECK no-neg; no altera enums ni datos). **Reconciliación canon=persistencia** (API mapea): alert.type expone 4 canónicos + subtype en `context`; recommendation status `new`↔`active`, priority int↔enum. **Nota de superset:** los shapes de alerts/recommendations son **más ricos que el `openapi.yaml`** (paths 1:1, superset; `yaml` a reconciliar en Fase 7). Detalle: `docs/implementation/phase-5-summary.md`, `docs/audit/phase-5-{precheck,spec-readiness,openapi-implementation-audit,runtime-verification}.md`, `docs/api/phase-5-alerts-recommendations.md`.
> **Fase 6 — ✅ PASS (backend · dry-run) (2026-06-04, `feat/phase-6-device-control`).** Implementados y verificados (API **140/140** contra Neon real; 26 control) los **9 endpoints de control de dispositivos**, todos bajo JWT + `assertDeviceAccess`, **todos en dry-run** (sin downlink físico ni MQTT):
> - **POST `/devices/{deviceId}/control-actions`** (header `Idempotency-Key` opcional; RBAC `ROLES.operate`, viewer → 403) — **dry-run**: persiste `status='success'`+`dry_run=true`+`result{dry_run}`, API expone `status='dry_run'`; **capability** `switch=true` o **409 `DEVICE_NOT_CONTROLLABLE`**; **idempotente** por `(deviceId, idempotency_key)`; audit `control.requested`+`control.resolved`; `value/source/reason` en `payload`. → `{id,device_id,action,status:'dry_run',dry_run,value,source,reason,requested_at,resolved_at,result}`.
> - **GET `/devices/{deviceId}/control-actions`** (`read`; `status?`/`limit?`) — bitácora de acciones del device.
> - **GET `/devices/{deviceId}/control-state`** (`read`) — `{device_id, controllable, current_state(on|off|unknown), last_action, dry_run:true}`; estado **lógico/simulado** desde la última acción (**no físico confirmado**).
> - **POST/GET `/devices/{deviceId}/control-schedules`** + **PATCH `/control-schedules/{id}`** (RBAC `ROLES.operate`) — `action` turn_on/off (`set_limit` → 422); `name/value/cron/starts_at/ends_at` en `rule` jsonb; audit `control_schedule.created/updated`; **NO ejecuta** (sin scheduler).
> - **POST/GET `/devices/{deviceId}/consumption-limits`** + **PATCH `/consumption-limits/{id}`** (RBAC `ROLES.operate`) — `limit_type` power_w/energy_kwh_day/month → `limitPowerW`/`limitKwh`+`window`; `action` notify→`alert`/turn_off (`set_limit` → 422); `threshold>0` (422 si ≤0); audit `consumption_limit.created/updated`; **NO ejecuta** (solo persiste política).
>
>   **Migración aditiva 0003** (`control_actions.idempotency_key` text + `dry_run` bool default true + UNIQUE parcial `(device_id, idempotency_key)`; no altera enums ni datos). **Reconciliación canon=persistencia** (API mapea): action `type`; status `dry_run` derivado; `value/source/reason` en `payload`; schedule en `rule`; limit_type→limitPowerW/limitKwh+window; action notify↔alert. **Nota de superset:** los shapes de control son **más ricos que el `openapi.yaml`** (paths base 1:1; GET de colección y PATCH son extensión contract-first del mismo recurso; `yaml` a reconciliar en Fase 7). **Toda la fase es dry-run** (sin comando físico/MQTT downlink; `control-state` lógico; schedules/limits no ejecutan); precondiciones para downlink real en `docs/security/phase-6-control-safety.md`. Detalle: `docs/implementation/phase-6-summary.md`, `docs/audit/phase-6-{precheck,spec-readiness,control-data-model-audit,openapi-implementation-audit,runtime-verification}.md`, `docs/api/phase-6-device-control.md`.
> **Diferidos (Fase 7+ / fase futura):** downlink MQTT productivo + comandos físicos + `resolveAction` async + automatización/smart-control + scheduler ejecutor (control real); `NotificationService` y UI productiva (`/alerts`, `/breakdown`, `/control`, `/smart-control`) fase posterior; Bills CRUD/UI y proyecciones (fase posterior). Estos grupos/capacidades están en el contrato pero **aún no implementados** (o solo en dry-run). El **frontend** (shell/UI en vivo de dashboard/reports/breakdown/alerts/control) es entrega posterior; la demo sigue bajo DEMO_MODE (web client `energyApi.*`/`insightsApi.*`/`controlApi.*` preparatorios, no usados por la UI).
> **Desviaciones documentadas:** (Fase 2) password con bcryptjs (12 rounds) en vez de Argon2id del canon; JWT único a 7d (refresh rotado + throttling → Fase 7). (Fase 3) `event_hash` usa `device_id` (no `kit_qr`/`device_ref`); telemetría no audita (volumen); agregación inline sin `cost_clp`; device auth = JWT de usuario (API key/kit-scope → Fase 7); MQTT productivo fuera de alcance (`iot-bridge` en dry-run). (Fase 4) costeo **solo energía** (sin cargo fijo/demanda/punta-valle/horario); tiempos **UTC** (tz local → fase posterior); breakdown solo dispositivos medidos (no NILM); respuestas **superset** del `openapi.yaml`. (Fase 5) evaluación sin scheduler real (función interna); sin `estimatedImpactClp` en alertas (impacto CLP solo en recomendaciones); ahorro kWh heurístico (10% del exceso observado); respuestas **superset** del `openapi.yaml`. (Fase 6) **control en dry-run** (sin comando físico/MQTT downlink, sin `resolveAction` async); `control-state` **lógico/simulado** (no físico confirmado); schedules/limits **no ejecutan** (sin scheduler); `set_limit`-as-schedule/limit → 422 (diferido); respuestas **superset** del `openapi.yaml`.
...
| Endpoint | Método | Rol requerido | FR | Criterio de aceptación (resumen) |
|---|---|---|---|---|
| `/devices/{deviceId}/control-actions` | POST | operator, admin, owner | FR-CTRL-001/002/008/009 | Con permiso → ControlAction; viewer → 403 + audit. **Fase 6 = dry-run** (`status='dry_run'`, sin downlink/resolución async; ver nota de estado). |
| `/devices/{deviceId}/control-actions` | GET | viewer+ | FR-CTRL-007 | Bitácora de acciones del device (`status?`/`limit?`). **(Fase 6, superset)** |
| `/devices/{deviceId}/control-schedules` | POST/GET | operator, admin, owner / viewer+ | FR-CTRL-004 | Crea/lista ControlSchedule. **Fase 6 = persiste política, no ejecuta** (`set_limit` → 422). |
| `/control-schedules/{id}` | PATCH | operator, admin, owner | FR-CTRL-004 | Actualiza ControlSchedule (audit). **(Fase 6, superset)** |
| `/devices/{deviceId}/consumption-limits` | POST/GET | operator, admin, owner / viewer+ | FR-CTRL-005/006 | Crea/lista límite (kWh o W) con pre-alerta y acción al exceder. **Fase 6 = persiste política, no ejecuta** (`threshold>0`; `set_limit` → 422). |
| `/consumption-limits/{id}` | PATCH | operator, admin, owner | FR-CTRL-005/006 | Actualiza límite (audit). **(Fase 6, superset)** |
| `/devices/{deviceId}/control-state` | GET | viewer+ | FR-CTRL-003 | Estado on/off/unknown + última acción. **Fase 6 = estado lógico/simulado** (no físico confirmado). |

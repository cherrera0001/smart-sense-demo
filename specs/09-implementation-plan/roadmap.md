...
> **Estado: ✅ PASS / implementada (2026-06-04, `feat/phase-7-hardening`)** — Hardening transversal cerrado sobre Neon real (credencial **rotada**). **Seguridad:** rotación REAL del secreto Neon (`ALTER ROLE`, vieja invalidada; nueva solo en `packages/db/.env` gitignored — `docs/security/phase-7-secret-rotation.md`); secret-scan limpio (`pnpm security:scan-secrets` exit 0); `@fastify/helmet` (`security-headers.ts`, `nosniff` verificado), `@fastify/cors` (`cors.ts`, `CORS_ORIGINS`; prod rechaza wildcard), `@fastify/rate-limit` (`rate-limit.ts`, global 300/min + estricto en `/auth/login|register` y `POST control-actions`; **429** verificado); `config/env.ts` rechaza JWT débil y CORS wildcard en prod; `config/logger.ts` pino con redacción. **Auth hardening:** refresh-token rotation (**migración aditiva 0004** `refresh_tokens` aplicada a Neon); access 15m + refresh 7d con `jti`+`token_hash` sha256; `POST /auth/refresh` rota; reuso de refresh revocado → 401 + revoca árbol; logout revoca; `register`/`login` devuelven `token`+`refresh_token` (shape conservado); access sin `passwordHash`. **Health:** `/health` y `/healthz` (liveness) + `/readyz` (readiness `SELECT 1`, 503 degraded), sin secretos, verificados contra Neon. **CI/CD:** `.github/workflows/ci.yml` (postgres:16 efímero, sin Neon; `secret-scan→typecheck→lint→build api/web/iot-bridge→migrate:deploy+seed+test:db:external`); Dockerfiles api/web multi-stage + `.dockerignore`; scripts root `security:scan-secrets`/`smoke:api`/`ci:verify`/`ci:test:db`. **Observabilidad:** `docs/observability/phase-7-observability.md` (logging redactado, request-id, `/metrics` Prometheus diferido). **Smoke:** `pnpm smoke:api` → 7 pasos OK contra API real + Neon. **Tests:** API **156/156** (+security 7, health 2, auth-refresh 7), iot-bridge **23/23**, DB **18/18**; typecheck/build:api/build:web (12 rutas DEMO_MODE)/lint verdes (1 warning preexistente `Step2PairingLeds.tsx:35`). **Gates Fase 7:** GATE-SEC-002..005, GATE-OPS-001/002, GATE-CI-001, GATE-E2E-001 = **PASS**; **GATE-DEPLOY-001 = BLOCKED por entorno** (Docker no disponible local; Dockerfiles listos para CI — no FAIL). **No se agregaron features de negocio; `apps/web` sigue DEMO_MODE.** Detalle: `docs/implementation/phase-7-summary.md`, `docs/audit/phase-7-{precheck,runtime-verification,openapi-implementation-audit}.md`, `docs/security/phase-7-secret-rotation.md`, `docs/observability/phase-7-observability.md`. **Roadmap F0–F7 COMPLETO.**
>
> **Fase 8 (fase futura, requiere autorización):** despliegue productivo real (env de prod, backups/DR, retención) + **downlink IoT físico** del control (canal IoT autenticado por kit, ACK/timeout, rollback, rate-limit reforzado, según `docs/security/phase-6-control-safety.md`) + `/metrics` Prometheus / tracing OpenTelemetry + reconciliación final del `openapi.yaml` (NFR-045).
> **Actualización (Fase 8.4):** el despliegue de la **API** se materializó como **Vercel Serverless** (no imágenes Docker en host persistente); Railway/Render/Fly/VPS/Docker como host de la API quedan **OBSOLETOS**. Ver `docs/deployment/phase-8-4-vercel-serverless-api.md`.
...
> **Estado Fase 8.4: ✅ API en Vercel Serverless ("todo en Vercel") (2026-06-08, `release/smartsense-f0-f7`).** La API (`apps/api`, Fastify) se desplegó como **Vercel Serverless Function** — proyecto Vercel **aislado** `smartsense-api-v2`, alias estable **`https://smartsense-api-v2.vercel.app`**, SSO/Deployment Protection **desactivado** (API backend pública; seguridad por JWT+RBAC propios). La app Fastify se monta en un handler serverless (`api/index.ts`) y se pre-empaqueta con esbuild (`scripts/bundle-api.mjs`: `@smartsense/*` inlineados, `@prisma/client` external, `binaryTargets` `rhel-openssl-3.0.x`); `DATABASE_URL` pooled Neon. **Verificado:** `GET /health` 200, `GET /readyz` 200 (`db: ok`), **smoke E2E remoto 7/7 PASS**; CI verde en el último push (success), commit `d87765b`; **PR #1 sin merge**; producción antigua `smart-sense-demo` NO tocada. **Esto SUSTITUYE (OBSOLETO/superseded) a Railway/Render/Fly/VPS/Docker como host de la API** — las notas 8.1/8.2/8.3 de "API staging READY-BLOCKED por credenciales Railway" quedan **superseded**. **Limitaciones serverless:** rate-limit desactivado en la función (endurecer en edge/WAF), sin WebSocket, cold starts. **Web** (`smartsense-web-v2`) sigue en build READY pero en **DEMO_MODE** (frontend productivo = fase futura). **GATE-DEPLOY-001/002 + GATE-E2E-001 remoto = PASS.** Pendiente de Fase 8 (cutover): **cutover web** de producción + merge autorizado. Detalle: `docs/deployment/phase-8-4-vercel-serverless-api.md`.
> **Fase 8.5 (Spec Compliance Gate):** reconciliación documental de specs de gobierno con la realidad F0–F7 + 8.4 (API Vercel Serverless; `refresh_tokens` #22; enums nativos PostgreSQL). Sin cambios de código ni de `openapi.yaml`.
>
> **Estado Fase 8.3: 🟡 PARTIAL robusto (2026-06-06, `release/smartsense-f0-f7`) — API staging OBSOLETO (superseded por Fase 8.4 Vercel Serverless).** **Paquete de staging ejecutable ✅** — `render.yaml` (Render Blueprint, `smartsense-api-staging`, start **`tsx`**, healthCheck `/health`), `scripts/deploy-railway-staging.{sh,ps1}` (no interactivos), `scripts/verify-staging.mjs` + script root `verify:staging` (`STAGING_API_URL` → `/health` + `/readyz` + smoke E2E). **Web v2 build ✅** (PASS verificado con logs: `Next.js 15.5.19`, `Compiled successfully`, `Build Completed`; **acceso BLOCKED-by-protection** 401, pasos UI en Settings → Deployment Protection del proyecto aislado). **API staging URL ⛔ READY-BLOCKED por credenciales** (railway/fly/render/docker no disponibles/no auth — no se desplegó, no se inventó); de ella depende el **smoke remoto ⛔**. **PR #1 comentado ✅** (sin merge). **Producción NO tocada ✅.** Aclaración: el build fallido `No Next.js` es de `smart-sense-demo` (branch-previews desde la raíz), inofensivo, no se toca. Docs: `docs/audit/phase-8-3-{precheck,vercel-web-v2-access,api-staging-status,pr-update}.md`, `docs/deployment/phase-8-3-{render-blueprint,railway-staging,vps-api-staging}.md`.
>
> **Estado Fase 8.2: 🟡 PARTIAL (2026-06-06, `release/smartsense-f0-f7`, HEAD `8244169`).** **Web v2 preview ✅** (Vercel **aislado** `smartsense-web-v2`, scope `cherrera0001s-projects`, `apps/web` linkeado, `NEXT_PUBLIC_DEMO_MODE=true`, deploy **READY**; HTTP 401 = Deployment Protection → acceso tras owner-auth, **no** fallo de build). **API staging ⛔ READY-BLOCKED por credenciales:** Railway `whoami` Unauthorized, `RAILWAY_TOKEN` ausente (bash+win) — **no se desplegó, no se inventó**; de ella depende el **smoke remoto ⛔**. **PR #1 comentado ✅** (sin merge). **Producción NO tocada ✅** (`smart-sense-demo` / `smartsense.c4a.cl` intactos, `master` sin cambios). Docs: `docs/audit/phase-8-2-{execution-precheck,railway-auth,vercel-web-v2-preview,api-staging-railway,pr-update}.md`, `docs/deployment/phase-8-2-{api-staging-manual-render,api-staging-manual-vps,production-cutover-runbook}.md`.
>
> **Estado Fase 8.1: 🟡 READY-BLOCKED (2026-06-06, `release/smartsense-f0-f7`).** Rama de release F0–F7 **pusheada**, CI on-push **PASS** y **PR #1** de revisión abierto; **deploy de API a staging y Vercel preview PENDIENTES** por falta de credenciales de hosting / autorización. Solo documentación + actualización de specs (no se tocó código).
>
...
- **Criterio de cierre (→ PASS):** rama pusheada + CI verde + deploy con health/readyz/smoke remoto OK. **Actualmente (Fase 8.4):** rama pusheada ✅, CI PASS ✅, PR #1 de revisión ✅; **deploy de API ✅ en Vercel Serverless** (`smartsense-api-v2.vercel.app`, `/health`+`/readyz` 200, smoke remoto 7/7 PASS) — sustituye al deploy en host persistente (Railway/Render/Fly/VPS = OBSOLETOS). **Pendiente:** **cutover web** de producción (mover dominio / Root Directory `apps/web`) + merge autorizado. Producción antigua no tocada.
...
| 8 | Release / CI / Deployment — **🟢 API en Vercel Serverless (8.4)** (rama pusheada + CI PASS + PR #1 sin merge; **API ✅ desplegada** `smartsense-api-v2.vercel.app`, `/health`+`/readyz` 200, smoke remoto 7/7; **web v2 build ✅** DEMO_MODE; Railway/Render/Fly/VPS/Docker = **OBSOLETOS**; pendiente: cutover web prod) | 1–7 | N/A (release/operación) |

> **Roadmap F0–F7 COMPLETO** (2026-06-04). Las 8 fases (0, 0.5, 1, 1.2, 1.4, 2, 3, 4, 5, 6, 7) cerradas en PASS. **Fase 8.2** (despliegue controlado, 2026-06-06) = **PARTIAL**: **web v2 preview build ✅** (Vercel aislado `smartsense-web-v2` READY; acceso tras owner-auth, 401 = Deployment Protection, no fallo de build); **API staging ⛔ READY-BLOCKED por credenciales** (Railway `whoami` Unauthorized, `RAILWAY_TOKEN` ausente bash+win — no se desplegó, no se inventó) y de ella depende el smoke remoto ⛔; **PR #1 comentado (sin merge)**; **producción NO tocada**. Desbloqueo de API: `railway login` o `RAILWAY_TOKEN` (runbooks Railway/Render/VPS listos). Fase 8 sigue **PARTIAL hacia PASS** (cutover en `docs/deployment/phase-8-2-production-cutover-runbook.md`). **Fase 8.3** (paquete de staging ejecutable, 2026-06-06) = **PARTIAL robusto** (OBSOLETO en su parte de host: superseded por 8.4): `render.yaml` + `scripts/deploy-railway-staging.{sh,ps1}` + `verify:staging` (runtime `tsx`) se conservan como historia; web v2 build PASS. Gates añadidos: GATE-DEPLOY-002..005 (`08-quality/runtime-gates.md`). **Fase 8.4** (2026-06-08) = **API en Vercel Serverless** (`smartsense-api-v2.vercel.app`): la API ya no se despliega en host persistente (Railway/Render/Fly/VPS/Docker = **OBSOLETOS**); `/health`+`/readyz` 200, smoke remoto 7/7 PASS, CI verde (commit `d87765b`), PR #1 sin merge, producción antigua intacta → GATE-DEPLOY-001/002 + GATE-E2E-001 remoto = **PASS**. **Fase 8.5** = Spec Compliance Gate (reconciliación documental). Pendiente de Fase 8: **cutover web** de producción + frontend productivo. Detalle: `docs/deployment/phase-8-4-vercel-serverless-api.md`.
